...
class IntcodeProgram {
    ...
    private final Map<Long, Long> initialState;
    private long index;
    ...
    IntcodeProgram(List<Long> state) {
        ...
        this.initialState = new HashMap<>(this.state);
        this.index = 0;
        this.relativeBase = 0;
    }

    long execute(Index input) {
        reset();
        boolean usedX = false;
        while (state.get(index) != EXIT_VALUE) {
            switch (OpCode.forRawCode(state.get(index))) {
                case INPUT:
                    if(usedX) {
                        state.put(referenceToValue(1), input.getY());
                    } else {
                        state.put(referenceToValue(1), input.getX());
                    }
                    usedX = !usedX;
                    index += 2;
                    break;
